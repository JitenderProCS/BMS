...
namespace BMS_New.Models.Infrastructure
{
    public class SQLHelper
    {
        ...
        public static string GetConnString()
        {
            ...
            string key = CryptoEngine.Decrypt(ConfigurationManager.AppSettings["ConnectionString"].ToString(), true);// @"Data Source=WIN-P99G4H9GTJL;Initial Catalog=MIS_JF;Persist Security Info=True;User ID=sa;Password=P@ssw0rd";// (string)settingsReader.GetValue("connectionString", typeof(String));
            connectionString = key;
            return connectionString;
        }
        public static string GetDBName()
        {
            return CryptoEngine.Decrypt(ConfigurationManager.AppSettings["DBName"].ToString(), true);
        }
        public static string[] getADstring()
        {
            try
            {
                ...
                while (!(objStreamReader.Peek() == -1) & k < 7)
                {
                    if (k == 1)
                    {
                        AD_DOMAIN = "LDAP://" + CryptoEngine.Decrypt(objStreamReader.ReadLine(), true) + ".com";
                    }
                    else if (k == 2)
                    {
                        AD_USER = CryptoEngine.Decrypt(objStreamReader.ReadLine(), true);
                    }
                    else if (k == 3)
                    {
                        AD_PSWRD = CryptoEngine.Decrypt(objStreamReader.ReadLine(), true);
                    }
                    else if (k == 4)
                    {
                        AD_DOMAIN_JLLR = "LDAP://" + CryptoEngine.Decrypt(objStreamReader.ReadLine(), true) + ".com";
                    }
                    else if (k == 5)
                    {
                        AD_USER_JLLR = CryptoEngine.Decrypt(objStreamReader.ReadLine(), true);
                    }
                    else if (k == 6)
                    {
                        AD_PSWRD_JLLR = CryptoEngine.Decrypt(objStreamReader.ReadLine(), true);
                    }
                    k = k + 1;
                }
                objStreamReader.Close();
